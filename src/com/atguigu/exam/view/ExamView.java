...
public class ExamView {
	...
	public void displayItem(int no){
		if (no < 1 || no > itemService.TOTAL_ITEMS) return;
	...
	private void displayResults(char[] charAnswer) {
		...
		for (int i = 0; i < itemService.TOTAL_ITEMS; i++) {
			Item item = itemService.getItem(i + 1);
			System.out.println("第" + ((i < 9) ? " " : "") + (i + 1) + " 题      " + item.getAnswer() + "           " + charAnswer[i]);
		}
		System.out.println("恭喜，本次考试成绩为：" + score + " 分");System.out.println("恭喜，本次考试成绩为：" + score + " 分");
	}
}
